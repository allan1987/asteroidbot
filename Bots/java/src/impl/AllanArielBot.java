...
public class AllanArielBot extends BotBase {
	...
	private InputVariable posX, posY, velX, velY, diffX, diffY, velAng, angleToTarget;
...
//		gamestate.log("att = " + att);
		...
		long mPrincipal = Math.round(motorPrincipal.getOutputValue());// * Math.round(Math.sin(Math.toRadians(getAng()))));
		long mLadoFrente = Math.round(motorLadoFrente.getOutputValue());// * Math.round(Math.cos(Math.toRadians(getAng()))));
		long mLadoFundo = Math.round(motorLadoFundo.getOutputValue());// * Math.round(Math.cos(Math.toRadians(getAng()))));
		int tiro = 1;
		
		gamestate.log(/*"diffX = " + diffX.getInputValue() + ", diffY = " + diffY.getInputValue() + */" e angleToTarget = " + angleToTarget.getInputValue());// + ", hipotenusa = " + hipotenusa + "seno = " + seno.getInputValue());
